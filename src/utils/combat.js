 ...
 * @param {ChatMessage|null} [opts.message]
 * @param {number[]} [opts.preShieldedIndices]  hit indices whose shield checkbox is pre-checked
 */
async function openTAMSDamageDialog(target, {
    ...
    let dialogContent = `<p>${game.i18n.format("TAMS.Combat.ApplyingHitsTo", {count: locations.length, name: e(target.name)})}</p>${squadHtml}${coverHtml}`;
    ...
    foundry.applications.api.DialogV2.wait({
      window: { title: game.i18n.format("TAMS.Checks.ApplyDamageTo", {name: e(target.name)}) },
...
export async function tamsRenderChatMessage(message, html, data) {
    ...
    root.querySelectorAll(".tams-contest-check-roll").forEach(btn => {
      ...
      btn.addEventListener("click", async ev => {
        ...
        // Resolve the contesting actor
        let currentActor;
        if (game.user.isGM) {
          const contested = new Set((currentFlags?.contests ?? []).map(c => c.actorId));
    ...
    root.querySelectorAll('.tams-block').forEach(el => el.addEventListener("click", async ev => {
        ...
        const actor = actorUuid ? await fromUuid(actorUuid) : canvas.tokens.controlled[0]?.actor;
