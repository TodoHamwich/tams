        ...
        if (dcTotal < difficulty && canBoost) {
            ...
            const boostConfirmed = await new Promise(resolve => {
                ...
                new Dialog({
                    title: "Boost Roll?",
                    content: `
                        <p>Total <b>${dcTotal}</b> fails to beat DC <b>${difficulty}</b>.</p>
                        <div class="form-group">
                            <label>Resource to spend (1 point = +1 to roll)</label>
                    ...
                    buttons: {
                        ...
                        cancel: { label: "No Boost", callback: () => resolve(null) }
                    },
                    default: "boost"
                }).render(true);
            });
            ...
            if (boostConfirmed) {
                ...
                if (res && res.value >= amount) {
                    ...
                    dcTotal += amount;
                    critInfo = `<div class="tams-success">Success vs Diff ${difficulty} (Boosted by ${amount})</div>`;
                } else {
                    ui.notifications.warn("Not enough resource to boost!");
                }
            }
        }

        if (dcTotal >= (difficulty * 2)) {
            critInfo = `<div class="tams-crit success">CRITICAL SUCCESS! (Total ${dcTotal} >= 2x Diff ${difficulty})</div>`;
        } else if (dcTotal >= difficulty) {
            if (!critInfo) critInfo = `<div class="tams-success">Success vs Diff ${difficulty}</div>`;
        } else {
...
Hooks.on("renderChatMessage", (message, html, data) => {
    ...
    root.querySelectorAll(".tams-take-damage").forEach(el => el.addEventListener("click", async ev => {
      ...
      // 3) Finally, fall back to a controlled token
      ...
      if (isAoEHit && isSquadOrHorde) {
          ...
          squadHtml = `
            <div class="form-group" style="margin-bottom: 10px;">
                <label>Targets Hit in ${typeLabel} (1-${currentSize})</label>
                <input type="number" id="aoe-targets-hit" value="${initialMultiplier}" min="1" max="${currentSize}"/>
                <p style="color: #d35400; font-size: 0.85em;"><i>Each hit is multiplied by this number.</i></p>
            </div>
          `;
      }
      const defaultDmg = damageBase * initialMultiplier;

      let dialogContent = `<p>Applying <b>${locations.length}</b> hits to <b>${target.name}</b>:</p>${squadHtml}`;
        ...
        apply: { label: "Apply All Hits", callback: async (html) => {
              ...
              for (let i = 0; i < locations.length; i++) {
                  ...
                  if (limb.hasEquippedArmor) {
                      const it = target.items.get(limb.equippedArmorId);
                      if (it && it.type === 'armor') {
                          armorItems = [it];
                  ...
                  if (armor > 0 && (effective + overflow) < incoming) {
                      ...
                      if (limb.hasEquippedArmor) {
                          ...
                          const itemToDamage = armorItems.find(it => {
                              if (isAltArmor) {
                                  const pending = itemUpdates[it.id]?.[`system.limbs.${limbKey}.max`];
                                  const val = pending !== undefined ? pending : (it.system.limbs[limbKey]?.max || 0);
                                  return val > 0;
                              } else {
                                  const pending = itemUpdates[it.id]?.[`system.limbs.${limbKey}.value`];
                                  const val = pending !== undefined ? pending : (it.system.limbs[limbKey]?.value || 0);
                                  return val > 0;
                              }
                          });
                          if (itemToDamage) {
                              if (isAltArmor) {
                                  const pending = itemUpdates[itemToDamage.id]?.[`system.limbs.${limbKey}.max`];
                                  const currentVal = pending !== undefined ? pending : (itemToDamage.system.limbs[limbKey]?.max || 0);
                                  itemUpdates[itemToDamage.id] = { 
                                      ...(itemUpdates[itemToDamage.id] || {}),
                                      _id: itemToDamage.id, 
                                      [`system.limbs.${limbKey}.max`]: Math.max(0, currentVal - multiplier) 
                                  };
                              } else {
                                  const pending = itemUpdates[itemToDamage.id]?.[`system.limbs.${limbKey}.value`];
                                  const currentVal = pending !== undefined ? pending : (itemToDamage.system.limbs[limbKey]?.value || 0);
                                  itemUpdates[itemToDamage.id] = { 
                                      ...(itemUpdates[itemToDamage.id] || {}),
                                      _id: itemToDamage.id, 
                                      [`system.limbs.${limbKey}.value`]: Math.max(0, currentVal - multiplier) 
                                  };
                              }
                          }
                      } else {
    ...
    root.querySelectorAll('.tams-dodge').forEach(el => el.addEventListener("click", async ev => {
      ...
      if (!actor) return ui.notifications.warn('Select (or target) a token to Dodge.');
      ...
      if (attackerTotal > total) {
          ...
          for (let i = 0; i < hitsScored; i++) {
              let loc;
              if (attackerLocations[i]) {
                  loc = attackerLocations[i];
              } else if (targetLimb && targetLimb !== 'none') {
                  loc = limbOptions[targetLimb] || "Thorax";
              } else {
                  loc = await getHitLocation();
              }
              locations.push(loc);
          }
          ...
          damageInfo = `
            ...
            <div class="roll-row" style="margin-top: 5px;">
                <button class="tams-take-damage" data-damage="${attackerDamage}" data-armour-pen="${attackerArmourPen}" data-locations='${JSON.stringify(locations)}' data-is-aoe="${isAoEFromData ? '1' : '0'}">Apply Damage</button>
            </div>
          `;
      ...
      const msg = `
        <div class="tams-roll" data-attacker-raw="${attackerRaw}" data-attacker-total="${attackerTotal}" data-attacker-multi="${attackerMulti}" data-attacker-damage="${attackerDamage}" data-attacker-armour-pen="${attackerArmourPen}" data-actor-id="${actor.id}" data-raw="${raw}" data-capped="${capped}" data-behind="${isBehind ? '1' : '0'}" data-unaware="${isUnaware ? '1' : '0'}" data-first-location="${firstLocation}" data-is-ranged="${isRanged ? '1' : '0'}" data-target-limb="${targetLimb}" data-is-aoe="${isAoEFromData ? '1' : '0'}">
    ...
    root.querySelectorAll('.tams-retaliate').forEach(el => el.addEventListener("click", async ev => {
      ...
      if (!actor) return ui.notifications.warn('Select (or target) a token to Retaliate.');
      ...
      // Retaliation doesn't check for AOE #4
      if (isAoEFromData) {
          return ui.notifications.warn("Retaliation is not possible against AOE attacks.");
      }
      ...
      let armourPen = 0;
      if (weapon.type === 'weapon' && weapon.system.hasArmourPen) {
          armourPen = weapon.system.armourPenetration || 0;
      } else if (weapon.type === 'ability') {
          armourPen = weapon.system.armourPenetration || 0;
      }
      ...
      if (hitsScored > 0) {
          let firstRetLoc;
          if (defenderTargetLimb && defenderTargetLimb !== 'none') {
              firstRetLoc = limbOptions[defenderTargetLimb] || "Thorax";
          } else {
              firstRetLoc = await getHitLocation(raw);
          }
          retLocations.push(firstRetLoc);
          for (let i = 1; i < hitsScored; i++) {
              let loc;
              if (defenderTargetLimb && defenderTargetLimb !== 'none') {
                  loc = limbOptions[defenderTargetLimb] || "Thorax";
              } else {
                  loc = await getHitLocation();
              }
              locations.push(loc);
              retLocations.push(loc);
          }
      }
      ...
      if (isMutual) {
          ...
          for (let i = 0; i < hitsTaken; i++) {
              ...
              if (attackerLocations[i]) {
                  loc = attackerLocations[i];
              } else if (attackerTargetLimb && attackerTargetLimb !== 'none') {
                  loc = limbOptions[attackerTargetLimb] || "Thorax";
              } else {
                  loc = await getHitLocation();
              }
              defenseLocations.push(loc);
          }
          defenseDamageInfo = `
            ...
            <div class="roll-row" style="margin-bottom: 10px;">
                <button class="tams-take-damage" data-damage="${attackerDamage}" data-armour-pen="${attackerArmourPen}" data-locations='${JSON.stringify(defenseLocations)}' data-is-aoe="${isAoEFromData ? '1' : '0'}">Apply Hits to Defender</button>
            </div>
          `;
          ...
          for (let i = 1; i < hitsTaken; i++) {
              let loc;
              if (attackerTargetLimb && attackerTargetLimb !== 'none') {
                  loc = limbOptions[attackerTargetLimb] || "Thorax";
              } else {
                  loc = await getHitLocation();
              }
      ...
      const retButtons = isMutual ? `
          <button class="tams-take-damage" data-damage="${damage}" data-armour-pen="${armourPen}" data-locations='${JSON.stringify(retLocations)}' data-is-aoe="${isRetAoE ? '1' : '0'}">Apply Damage</button>
      ` : (hitsScored > 0 ? `
          <button class="tams-take-damage" data-damage="${damage}" data-armour-pen="${armourPen}" data-locations='${JSON.stringify(retLocations)}' data-is-aoe="${isRetAoE ? '1' : '0'}">Apply Damage</button>
          <button class="tams-dodge" data-raw="${raw}" data-total="${total}" data-multi="${multiVal}" data-location="${retLocations[0]}" data-damage="${damage}" data-armour-pen="${armourPen}" data-is-ranged="${isRanged ? '1' : '0'}" data-is-aoe="${isRetAoE ? '1' : '0'}" data-target-limb="${defenderTargetLimb}">Dodge</button>
          <button class="tams-retaliate" data-raw="${raw}" data-total="${total}" data-multi="${multiVal}" data-location="${retLocations[0]}" data-damage="${damage}" data-armour-pen="${armourPen}" data-is-ranged="${isRanged ? '1' : '0'}" data-is-aoe="${isRetAoE ? '1' : '0'}" data-target-limb="${defenderTargetLimb}">Retaliate</button>
